...
\item \textbf{Frecuencia:} antes de cada despliegue de una version nueva del modelo, y revision trimestral en produccion.
\end{itemize}
...
\item \textbf{Frecuencia:} en cada entrega que modifique el motor de alertas.
\end{itemize}
...
\item \textbf{Frecuencia:} en cada reentrenamiento del modelo.
\end{itemize}
...
\item \textbf{Frecuencia:} en cada entrega, como parte de las pruebas de aceptacion.
\end{itemize}
...
\item \textbf{Frecuencia:} reporte mensual mientras el sistema este en operacion.
\end{itemize}
...
\item \textbf{Responsable:} Danela Arteaga (especificacion); equipo tecnico (implementacion).
\item \textbf{Frecuencia:} al definir cada nuevo tipo de alerta.
\end{itemize}
...
\item \textbf{Postcondicion:} parcela visible en el listado general (RF-01); ningun registro de cultivo queda con valor "otros" sin especificar (RF-02).
\item \textbf{Flujo basico:}
  \begin{enumerate}
  ...
  \end{enumerate}
\item \textbf{Flujos alternativos / excepciones:}
  \begin{itemize}
  \item Si falta alguno de los 7 campos obligatorios, el sistema rechaza el guardado y marca los campos vacios (RF-01).
  \item Si el usuario intenta ingresar un cultivo fuera del catalogo cerrado, el sistema lo rechaza (RF-02).
  \item El usuario puede registrar la condicion climatica/de suelo en cualquier momento posterior (RF-16).
  \item El usuario puede registrar un analisis de suelo previo a la siembra (RF-35).
  \end{itemize}
...
\item \textbf{Requisitos relacionados:} RF-03.
...
\item \textbf{Flujo basico:}
  ...
  \end{enumerate}
\item \textbf{Flujos alternativos / excepciones:} si falta fecha, parcela o trabajador, el sistema rechaza el guardado (RF-03).
...
\item \textbf{Requisitos relacionados:} RF-04, RF-13, RF-14, RF-15.
...
\item \textbf{Flujos alternativos / excepciones:}
  \begin{itemize}
  \item Unidad de cosecha invalida para el cultivo: el sistema rechaza el registro (RF-04).
  \item El usuario registra una cantidad rechazada y su motivo (enfermedad, plaga, dano mecanico); el sistema excluye automaticamente esa cantidad del total utilizable (RF-13).
  \item Si la parcela es de exportacion, el usuario registra color de cinta, semana de enfunde y calidad de caja (RF-14).
  \end{itemize}
...
\item \textbf{Precondicion:} existen 2 o mas registros de cosecha para la parcela (RF-05); existen registros de ingresos y egresos del periodo (RF-06).
...
\item \textbf{Flujo basico:}
  ...
  \item El sistema consulta los registros de ingresos y egresos (automaticos por insumos + manuales por RF-26) del periodo.
  \item El sistema calcula la ganancia neta (ingresos menos egresos) y la muestra en el modulo financiero.
  \end{enumerate}
\item \textbf{Flujos alternativos / excepciones:}
  \begin{itemize}
  \item Si existen menos de 2 registros de cosecha, el sistema muestra "datos insuficientes" en vez de un promedio.
  \item El usuario puede registrar manualmente un ingreso o egreso adicional (concepto, monto, fecha, tipo), independiente del calculo automatico (RF-26).
  \end{itemize}
...
\begin{itemize}
\item \textbf{Actor(es) principal(es):} Administrador, Tecnico, Jornalero.
\item \textbf{Requisitos relacionados:} RF-07, RF-08, RF-30.
\item \textbf{Precondicion:} ninguna para registrar un insumo nuevo; un umbral definido para el insumo (RF-08).
...
\item \textbf{Flujo basico:}
  ...
  \end{enumerate}
\item \textbf{Flujos alternativos / excepciones:} el Tecnico puede configurar/editar el umbral minimo de stock de cada insumo en cualquier momento (RF-30).
\item \textbf{Reglas de negocio:} CU-05 dispara una alerta al cruzar el stock minimo (RF-08, RF-30); el stock nunca puede quedar negativo, el sistema debe advertir antes de permitirlo.
\end{itemize}
...
\item \textbf{Requisitos relacionados:} RF-09, RF-10, RF-31, RF-33.
...
\item \textbf{Flujo basico:}
  \begin{enumerate}
  \item El Sistema de IA analiza los datos de la parcela (historial de tratamientos y, opcionalmente, una foto subida por el usuario) y genera una sugerencia con justificacion y fuente de datos.
...
\item \textbf{Flujos alternativos / excepciones:}
  \begin{itemize}
  \item El usuario descarta la sugerencia de IA (RF-09).
  \item El usuario puede registrar formalmente su desacuerdo con una recomendacion, distinto de un simple confirmar/descartar, disponible para revision del equipo tecnico (RF-33).
  \item El sistema compara un valor recien ingresado contra el promedio historico de la parcela y advierte si es un valor atipico antes de guardar (RF-31).
  \end{itemize}
\item \textbf{Reglas de negocio:} ninguna alerta se aplica automaticamente sobre una parcela sin confirmacion del usuario responsable (regla central de RF-09, ligada a RNF-17/RNF-19); toda alerta debe mostrar su justificacion (maximo 60 palabras) y al menos una fuente de datos antes del boton de confirmacion.
\end{itemize}

\subsubsection{CU-07. Asignar y dar seguimiento a tareas}
\begin{itemize}
\item \textbf{Actor(es) principal(es):} Administrador, Tecnico, Jornalero.
\item \textbf{Requisitos relacionados:} RF-11, RF-12, RF-27, RF-28.
...
\item \textbf{Flujo basico:}
  ...
  \item El trabajador actualiza el estado de la tarea (pendiente $\rightarrow$ en progreso $\rightarrow$ bloqueada $\rightarrow$ completada) y puede adjuntar una observacion en texto libre (RF-28).
  \item El trabajador puede consultar en cualquier momento la vista de sus propias tareas pendientes (RF-27).
  \end{enumerate}
\item \textbf{Flujos alternativos / excepciones:} si la parcela tiene marcado un riesgo laboral (CU-11), el sistema muestra la advertencia de EPP antes de confirmar la asignacion.
\item \textbf{Reglas de negocio:} el filtro de estado debe mostrar exclusivamente las tareas en ese estado, sin mezclar parcelas o trabajadores; la observacion de una tarea completada permanece visible al ver el detalle de la tarea (RF-28).
\end{itemize}
...
\item \textbf{Precondicion:} existen datos en el rango de fechas seleccionado.
\item \textbf{Postcondicion:} reporte generado y descargable.
\item \textbf{Flujo basico:}
  \begin{enumerate}
  \item El usuario selecciona un rango de fechas y una o mas parcelas.
  \item El sistema consolida produccion, costos y perdidas por plagas, y rendimiento por parcela/periodo.
  \item El sistema genera graficos de barras y de pastel a color.
  \item El usuario exporta el reporte.
  \end{enumerate}
\item \textbf{Flujos alternativos / excepciones:} si no existen datos en el rango seleccionado, el sistema informa "sin datos disponibles" en vez de un reporte vacio.
\item \textbf{Reglas de negocio:} el reporte generado debe reflejar exactamente la suma de los registros del rango de fechas seleccionado, sin redondeo no declarado.
\end{itemize}

\subsubsection{CU-09. Autenticacion y control de acceso}
\begin{itemize}
\item \textbf{Actor(es) principal(es):} Todos (Administrador, Tecnico, Jornalero).
...
\item \textbf{Flujos alternativos / excepciones:}
  \begin{itemize}
  \item Credenciales invalidas: el sistema niega el acceso.
  \item Un usuario con rol "jornalero" que intenta acceder al modulo financiero: el sistema niega el acceso (RF-20).
  \item El trabajador puede solicitar acceso, rectificacion o eliminacion de sus propios datos personales en cualquier momento (derechos ARCO+, RF-23).
  \end{itemize}
\item \textbf{Reglas de negocio:} CU-09 es un Must-have regulatorio: ningun modulo del sistema es accesible sin autenticacion y sin consentimiento LOPDP registrado; el rol determina de forma exhaustiva el conjunto de funciones visibles (principio de minimo privilegio).
\end{itemize}

\subsubsection{CU-10. Visitas tecnicas}
\begin{itemize}
\item \textbf{Actor(es) principal(es):} Tecnico.
\item \textbf{Requisitos relacionados:} RF-21.
\item \textbf{Precondicion:} la parcela existe.
\item \textbf{Postcondicion:} visita registrada con hallazgos y, si aplica, marca de seguimiento.
\item \textbf{Flujo basico:}
  \begin{enumerate}
  \item El Tecnico selecciona la parcela a visitar.
  \item Registra fecha, hallazgos y si requiere seguimiento.
  \item El sistema guarda la visita y la vincula al historial de cumplimiento de la parcela.
  \end{enumerate}
\item \textbf{Flujos alternativos / excepciones:} si se encuentra incumplimiento, el sistema sugiere registrar la capacitacion o el EPP relacionado (ver CU-11, CU-13).
...
\item \textbf{Flujo basico:}
  ...
  \end{enumerate}
\item \textbf{Flujos alternativos / excepciones:} el Administrador adjunta el certificado de salud del trabajador al registro (RF-24, Res. AGROCALIDAD 183 Art. 33-34).
...
\begin{itemize}
\item \textbf{Actor(es) principal(es):} Jornalero.
\item \textbf{Requisitos relacionados:} RF-17.
\item \textbf{Precondicion:} usuario autenticado.
...
\item \textbf{Flujo basico:}
  ...
  \item El sistema almacena el reporte y lo marca para revision/triage del Tecnico.
  \end{enumerate}
\item \textbf{Flujos alternativos / excepciones:} si el reporte describe una posible plaga cuarentenaria, el sistema sugiere escalarlo como aviso fitosanitario formal (CU-14).
...
\begin{itemize}
\item \textbf{Actor(es) principal(es):} Administrador.
...
\item \textbf{Flujo basico:}
  ...
  \end{enumerate}
\item \textbf{Flujos alternativos / excepciones:} si falta una capacitacion obligatoria, el sistema bloquea la solicitud de renovacion hasta que se registre (RF-36, RF-39).
...
\item \textbf{Flujo basico:}
  ...
  \item El sistema genera un aviso fitosanitario.
  \item Un Tecnico confirma el sintoma.
  \item El sistema deja el aviso disponible para presentacion formal ante AGROCALIDAD y registra el evento de bioseguridad de ingreso/salida de la visita (RF-38).
  \end{enumerate}
\item \textbf{Flujos alternativos / excepciones:} el Tecnico descarta el sintoma tras la inspeccion; el aviso no se envia, pero queda registrado como descartado, para trazabilidad.
